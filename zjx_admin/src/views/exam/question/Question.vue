<template>
  <div class="app-container">
    ...
    <div style="margin-bottom: 10px">
      ...
      <a :href="BASE_API + '/admin/exam/question/export'">
        <el-button size="mini" type="primary" icon="el-icon-download">下载数据</el-button>
      </a>
      <a href="/static/excelTemplate/QuestionList模板.xlsx">
      ...
      <el-upload
        ...
        :action="BASE_API + '/admin/exam/question/upload'"
        class="upload-demo"
    ...
    <el-drawer
      ...
      <question-search @click-get-data="filterData"/>
    </el-drawer>
    ...
    <el-table :data="list" border stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="title" label="试题题目,最多50汉字(100字节)" width="100" align="center"/>
      <el-table-column prop="type" label="试题类型 0:单选 1:多选 2:判断" width="100" align="center"/>
      <el-table-column prop="answer" label="试题答案" width="100" align="center"/>
...
import QuestionSearch from '@/views/exam/question/QuestionSearch'
import QuestionDetail from '@/views/exam/question/QuestionDetail'

export default {

  components: { QuestionDetail, QuestionSearch },
  ...
  methods: {
    ...
    // 调用api模块，加载  列表数据
    getData() {
      questionApi.pageList(this.page, this.limit, this.searchObj).then(response => {
        this.list = response.data.list
        this.total = response.data.total
      })
    },
    filterData(data) {
      questionApi.pageList(this.page, this.limit, data).then(response => {
        this.list = response.data.list
        this.total = response.data.total
      })
    },
    ...
    batchRemove() {
      ...
      this.$confirm('此操作将永久批量删除数据, 是否继续?', '提示', {
        ...
        // 调用api
        return questionApi.batchRemove(idList)
      }).then((response) => {
        this.getData()
